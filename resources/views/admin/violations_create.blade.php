            ...
            <form action="{{ route('admin.violations.store') }}" method="POST" class="form-admin">
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="date" class="form-control" id="violation_date" name="violation_date" value="{{ old('violation_date', date('Y-m-d')) }}" required>
                    </div>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-control" id="violation_type" name="violation_type" required>
                            ...
                            <option value="Vượt đèn đỏ" {{ old('violation_type') == 'Vượt đèn đỏ' ? 'selected' : '' }}>Vượt đèn đỏ</option>
                            <option value="Chạy quá tốc độ" {{ old('violation_type') == 'Chạy quá tốc độ' ? 'selected' : '' }}>Chạy quá tốc độ</option>
                            <option value="Không đội mũ bảo hiểm" {{ old('violation_type') == 'Không đội mũ bảo hiểm' ? 'selected' : '' }}>Không đội mũ bảo hiểm</option>
                            <option value="Đỗ xe sai quy định" {{ old('violation_type') == 'Đỗ xe sai quy định' ? 'selected' : '' }}>Đỗ xe sai quy định</option>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="number" class="form-control" id="fine_amount" name="fine_amount" value="{{ old('fine_amount') }}" required>
                    </div>
                    ...
                    <div class="col-md-6 mb-3 payment-method-div" style="display: {{ old('payment_status') == 'Paid' ? 'block' : 'none' }};">
                        <label for="payment_method" class="form-label">Phương thức thanh toán</label>
                        <select class="form-control" id="payment_method" name="payment_method">
                            <option value="">Chọn phương thức thanh toán</option>
                            <option value="Online" {{ old('payment_method') == 'Online' ? 'selected' : '' }}>Thanh toán online</option>
                            <option value="Offline" {{ old('payment_method') == 'Offline' ? 'selected' : '' }}>Thanh toán trực tiếp</option>
                        </select>
                    </div>
